...
function SetupCanvas() {
    ...
    ctx.strokeRect(4, 4, 140, 231);

    tetrisLogo = new Image(80, 27);
    ...
    ctx.font = '11px Arial';
    // Draw highscore
    ctx.fillText("HIGHSCORE", 150, 50);
    ctx.strokeRect(150, 52, 80, 12);
    ctx.fillText(highScore.toString(), 152, 62);

    // Draw score
    ctx.fillText("SCORE", 150, 77);
    ctx.strokeRect(150, 80, 80, 12);
    ctx.fillText(score.toString(), 152, 90);

    // Draw level
    ctx.fillText("LEVEL", 150, 105);
    ctx.strokeRect(150, 107, 80, 12);
    ctx.fillText(level.toString(), 152, 117);

    // Draw next
    ctx.fillText("WIN / LOSE", 150, 132);
    ctx.strokeRect(150, 135, 80, 12);
    ctx.fillText(winOrLose, 152, 144);

    // // Draw controls
    ctx.fillText("CONTROLS", 150, 167);
    ctx.strokeRect(150, 170, 80, 64);
    ctx.font = '10px Arial';
    ctx.fillText("A: Move Left", 152, 181);
    ctx.fillText("D: Move Right", 152, 194);
    ctx.fillText("S: Move Down", 152, 206);
    ctx.fillText("W: Rotate Right", 152, 219);
    ctx.fillText("P: Pause", 152, 231);
...
function CheckForCompletedRows() {
    ...
    if (rowsToDelete > 0) {
        ...
        if (score > localStorage.getItem('highscore')) {
            ...
            ctx.fillStyle = 'white';
            ctx.fillRect(150, 54, 70, 10);
            ctx.fillStyle = 'gray';
            ctx.fillText(score.toString(), 152, 62);
        }
        // update score
        ctx.fillStyle = 'white';
        ctx.fillRect(151, 81, 70, 10);
        ctx.fillStyle = 'gray';
        ctx.fillText(score.toString(), 152, 90);
